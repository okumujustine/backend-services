...
from .serializers import GenerateSQLSerializer
...
def ai_query_db(request, db_id_name):
    if request.method == "POST":
        ...
        serializer = GenerateSQLSerializer(data=request.data)
        

        if not serializer.is_valid():
            return Response({"result": {"error": serializer.errors }}, status=status.HTTP_400_BAD_REQUEST)
